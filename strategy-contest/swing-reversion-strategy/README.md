...
## Logic
1.  **Anchor**: Calculates a 50-period Simple Moving Average (SMA).
2.  **Grid**: Defines buy levels below the SMA at fixed percentage steps (e.g., -2%, -4%, -6%).
3.  **Entry**: Buys when price dips to these levels, scaling into weakness.
4.  **Exit**: Sells all positions when price reverts to the SMA (Mean Reversion) or exceeds it.
5.  **Risk Management**:
